...
- **الأدمن الرئيسي** يُزرع مباشرة في قاعدة بيانات فايرستور (Firestore) كبروفايل Google OAuth
- **بيانات الأدمن:**
  - البريد الإلكتروني: admin@depth-agency.com
  - الاسم: علي الربيعي (Ali Al-Rubaie)
  - الهاتف: 07719956000
  - الدور: مدير وكالة ديب (Depth Agency Manager)
- الأدمن الوحيد حالياً، ولا يمكن إضافة أدمنز آخرين في 2.0
...
```
CreatorPrice = BasePrice × OwnershipFactor × ProcessingMod × ExperienceMod × EquipmentMod × RushMod × LocationMod + LocationAddition
```

#### للمبدع بدون معدات (يستخدم معدات الوكالة):
```
CreatorPrice = (BasePrice × 0.9) × ProcessingMod × ExperienceMod × RushMod × LocationMod + LocationAddition
```
