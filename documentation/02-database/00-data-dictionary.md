...
| addedBy       | string       | لا    | معرف من أضافه              | sa_123abc      | FK → Admin |
...
| isSeeded      | boolean      | لا    | مُلغى في V2.0              | —              | تمت إزالة مفهوم "الأدمن المزروع"؛ الإنشاء يتم خارج النظام (سكربت زرع) |
...
| paymentTerms  | enum         | نعم   | شروط الدفع                 | advance_50     | advance_50/advance_100/net_15/net_30 |
| isActive      | boolean      | نعم   | الحساب نشط                | true           |              |
| createdAt     | timestamp    | نعم   | تاريخ الإنشاء              | 2025-08-20     |              |
| updatedAt     | timestamp    | نعم   | آخر تحديث                  | 2025-08-21     |              |
...
### 3.1 المشروع (Project)
> الكيان المركزي للمشاريع مع نظام التسعير المتقدم والمعاملات - مدعوم بنظام المهام المتعددة V2.1

> ⚠️ **ملاحظة مهمة:** الأسعار (creatorPrice) لا تظهر للمبدع إلا بعد موافقة الأدمن على المشروع. كما أن (clientPrice) لا يظهر للمبدع إطلاقاً في أي مرحلة.

| الحقل         | النوع         | مطلوب | وصف                        | مثال           | قيود         |
|---------------|--------------|-------|----------------------------|----------------|--------------|
| id                | string (uid) | نعم   | معرف المشروع               | p_123abc       | فريد         |
| clientId          | string       | نعم   | معرف العميل                | cl_123abc      | FK → Client  |
| creatorId         | string       | لا    | معرف المبدع المعين         | c_123abc       | FK → Creator |
...
| industryId        | string       | لا    | معرف المجال الصناعي        | ind_fashion    | FK → Industry |
| title             | string       | نعم   | عنوان المشروع             | تصوير منتجات   |              |
| description       | string       | نعم   | وصف المشروع               | تصوير 20 منتج  |              |
| requirements      | string       | لا    | متطلبات خاصة              | خلفية بيضاء     |              |
| status            | enum         | نعم   | حالة المشروع               | draft          | draft/pending/active/completed/cancelled |
| priority          | enum         | نعم   | أولوية المشروع            | normal         | low/normal/high/urgent |
| totalTasks        | int          | نعم   | عدد المهام الكلي           | 3              | ≥1           |
| completedTasks    | int          | نعم   | عدد المهام المكتملة       | 1              | ≥0           |
...
| agencyMarginPercent| float       | نعم   | نسبة هامش الوكالة (لقطة من الإعداد المركزي) | 0.20           | من إعداد واحد في لوحة الإدارة (مفتوح بلا حدود) |
...
| intelligentRecommendations| boolean| نعم| تم استخدام التوصية الذكية | true           |              |
| recommendationScore| float      | لا    | نقاط التوصية الذكية       | 0.85           | 0.0-1.0      |
| templateType      | enum         | نعم   | نوع القالب المعروض        | unified        | unified/role_based |
| attachments       | array        | لا    | مرفقات العميل             | [...]          | file URLs    |
| deliverables      | array        | لا    | المخرجات المطلوبة         | [...]          | strings      |
...
| completedAt       | timestamp    | لا    | تاريخ الإنجاز الكامل       | 2025-08-25     |              |
...
### 3.3 مهام المشاريع (Tasks)
> المهام الفردية داخل كل مشروع - نظام المشاريع متعددة المهام V2.1

| الحقل         | النوع         | مطلوب | وصف                        | مثال           | قيود         |
|---------------|--------------|-------|----------------------------|----------------|--------------|
| id            | string (uid) | نعم   | معرف المهمة                | t_123abc       | فريد         |
| projectId     | string       | نعم   | معرف المشروع               | p_123abc       | FK → Project |
| subcategoryId | string       | نعم   | معرف الفئة الفرعية         | sub_product    | FK → Subcategory |
| title         | string       | نعم   | عنوان المهمة               | تصوير منتج A   |              |
| description   | string       | نعم   | وصف المهمة                 | تصوير 5 زوايا  |              |
| quantity      | int          | نعم   | العدد/الكمية               | 5              | ≥1           |
| unitType      | enum         | نعم   | وحدة القياس                | piece          | piece/hour/session/page |
| processingLevel| enum        | نعم   | مستوى المعالجة            | intermediate   | basic/intermediate/advanced |
| priority      | enum         | نعم   | أولوية المهمة             | normal         | low/normal/high/urgent |
| status        | enum         | نعم   | حالة المهمة                | pending        | pending/in_progress/completed/cancelled |
| assignedCreatorId| string    | لا    | معرف المبدع المكلف        | c_123abc       | FK → Creator |
| estimatedHours| float        | لا    | الساعات المقدرة            | 2.5            | ≥0           |
| actualHours   | float        | لا    | الساعات الفعلية            | 3.0            | ≥0           |
| deliverables  | array        | لا    | مخرجات المهمة             | [...]          | file URLs    |
| pricing       | object       | لا    | تفاصيل التسعير            | {...}          | JSON         |
| dueDate       | timestamp    | لا    | الموعد النهائي             | 2025-08-25     |              |
| completedAt   | timestamp    | لا    | تاريخ الإنجاز              | 2025-08-24     |              |
| createdAt     | timestamp    | نعم   | تاريخ الإنشاء              | 2025-08-20     |              |
| updatedAt     | timestamp    | نعم   | آخر تحديث                  | 2025-08-21     |              |

### 3.4 توصيات المبدعين الذكية (CreatorRecommendations)
> نظام التوصيات الذكية لمطابقة المبدعين مع المشاريع SCR V2.1

| الحقل         | النوع         | مطلوب | وصف                        | مثال           | قيود         |
|---------------|--------------|-------|----------------------------|----------------|--------------|
| id            | string (uid) | نعم   | معرف التوصية               | rec_123abc     | فريد         |
| projectId     | string       | نعم   | معرف المشروع               | p_123abc       | FK → Project |
| creatorId     | string       | نعم   | معرف المبدع                | c_123abc       | FK → Creator |
| recommendationType| enum      | نعم   | نوع التوصية                | ai_algorithm   | ai_algorithm/manual/hybrid |
| score         | float        | نعم   | نقاط التطابق               | 0.92           | 0.0-1.0      |
| factors       | object       | نعم   | عوامل التقييم              | {...}          | JSON         |
| skillsMatch   | float        | نعم   | تطابق المهارات             | 0.95           | 0.0-1.0      |
| experienceMatch| float       | نعم   | تطابق الخبرة               | 0.88           | 0.0-1.0      |
| availabilityMatch| float     | نعم   | تطابق التوفر               | 1.0            | 0.0-1.0      |
| locationMatch | float        | نعم   | تطابق الموقع               | 0.85           | 0.0-1.0      |
| budgetMatch   | float        | نعم   | تطابق الميزانية            | 0.90           | 0.0-1.0      |
| industryMatch | float        | نعم   | تطابق المجال الصناعي       | 0.95           | 0.0-1.0      |
| status        | enum         | نعم   | حالة التوصية               | active         | active/accepted/rejected/expired |
| adminReview   | boolean      | نعم   | تمت المراجعة الإدارية      | false          |              |
| expiresAt     | timestamp    | نعم   | تاريخ انتهاء الصلاحية      | 2025-08-27     | +7 أيام      |
| createdAt     | timestamp    | نعم   | تاريخ الإنشاء              | 2025-08-20     |              |
| updatedAt     | timestamp    | نعم   | آخر تحديث                  | 2025-08-21     |              |
...
> جدول البذور للفئات الرئيسية - يُزرع عبر سكربت خارجي فقط
...
> عناصر البنود (invoiceItems) داخل كل فاتورة:
| الحقل | النوع | مطلوب | وصف | مثال | قيود |
|---|---|---|---|---|---|
| description | string | نعم | وصف المهمة | تصوير منتجات | ≤200 حرف |
...
- **كيانات المشاريع (4):** Project, ProjectRequests, Tasks, CreatorRecommendations  
...
- **كيانات النظام (8):** CreatorAvailability, Notifications, NotificationSettings, Reviews, OTPCodes, auditLogs, Invoices, Payments
- **الكيانات المرجعية (5):** SalariedEmployee, Industry, SubcategoryIndustryLink, FileUploads, MobileCache

### 🎯 **إجمالي:** 34 كيان منظم ونظيف (V2.1)

> 🏆 **تم تحديث الملف للنسخة V2.1!**  
> ✅ إضافة نظام المهام المتعددة (Tasks)  
> ✅ إضافة نظام التوصيات الذكية (CreatorRecommendations)  
> ✅ تحديث جدول المشاريع بالحقول الجديدة  
> ✅ دعم كامل للقوالب الموحدة والمجالات الصناعية  
> ✅ هيكلة متطورة ومتوافقة مع V2.1

---

**آخر تحديث:** 2025-09-04 (V2.1)  
**الحالة:** محدث ومتوافق مع نظام المشاريع متعددة المهام ✨
