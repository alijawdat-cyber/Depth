...
// import UnifiedUploader from "../features/portal/files/UnifiedUploader"; // TODO: Create new uploader
import { Button } from "@/components/ui/Button";
// Types are centralized in src/types; local state not used for these now
import WhatsAppButton from "@/components/ui/WhatsAppButton";
// import PendingApprovalScreen from "../features/portal/PendingApprovalScreen"; // TODO: Create new approval screen
// import WelcomeOnboarding from "../features/portal/WelcomeOnboarding"; // TODO: Create new onboarding
...
import { useProjects, useFiles, useApprovals } from "@/hooks/useClientData";
    ...
    try {
      const response = await fetch('/api/client/approvals', {
        ...
        body: JSON.stringify({
          ...
          notes: comment || '',
          feedback: feedback || '',
          requestedChanges: attachmentUrl ? attachmentUrl.split(/[\n,]+/).map(s => s.trim()).filter(Boolean) : undefined,
        }),
      });
  ...
  if (clientStatus === 'pending' || clientStatus === 'rejected') {
    return (
      <div className="min-h-screen bg-[var(--bg)] flex items-center justify-center">
        <div className="bg-[var(--card)] p-8 rounded-xl text-center max-w-md">
          <h2 className="text-xl font-semibold mb-4">في انتظار الموافقة</h2>
          <p className="text-[var(--text-muted)]">
            حسابك قيد المراجعة. سيتم إشعارك بمجرد الموافقة عليه.
          </p>
          <p className="text-sm text-[var(--text-muted)] mt-2">
            المستخدم: {session?.user?.email || ''}
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-[var(--bg)] flex items-center justify-center">
        <div className="bg-[var(--card)] p-8 rounded-xl text-center max-w-md">
          ...
          <p className="text-[var(--text-muted)]">
            مرحباً بك في لوحة تحكم العميل. ستجد هنا جميع مشاريعك وملفاتك.
          </p>
          <Button 
            onClick={() => {
          ...
          {tab === "files" && (
            <div className="space-y-6">
              ...
              <div id="uploaders" className={`${!activeProject ? 'opacity-50 pointer-events-none' : ''}`}>
                {/* TODO: Add UnifiedUploader component */}
                <div className="bg-[var(--bg)] p-4 rounded-lg border-2 border-dashed border-[var(--border)] text-center">
                  <p className="text-[var(--text-muted)]">رافع الملفات سيتم إضافته قريباً</p>
                  <p className="text-sm text-[var(--text-muted)] mt-1">Project ID: {activeProject?.id || 'N/A'}</p>
                </div>
              </div>
              {!activeProject && (
