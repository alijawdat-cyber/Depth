        ...
        <div id="mobile-menu" className="md:hidden border-t border-[var(--elev)] bg-[var(--bg)]/95 backdrop-blur px-4 py-3">
          ...
          <div className="flex items-center gap-2 mt-3">
            ...
            {status === 'authenticated' ? (
              ...
              <>
                <Link href={CTA_ITEMS.signin.href} onClick={() => setOpen(false)} className={clsx(buttonStyles({ variant: 'secondary', size: 'md' }), 'flex-1 text-center')}>{CTA_ITEMS.signin.label}</Link>
                <Link href={CTA_ITEMS.book.href} onClick={() => setOpen(false)} className={clsx(buttonStyles({ variant: 'primary', size: 'md' }), 'flex-1 text-center')}>{CTA_ITEMS.book.label}</Link>
              </>
            )}
          </div>
        </div>
